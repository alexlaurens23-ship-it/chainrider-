...
apps/
  web/                        Vite + vanilla TS. Hash-routed SPA: #/ home, #/map/:slug/:period, #/ride/:trackId, #/playground.
    ...
    src/auth.ts               Wallet sign-in (P6): Phantom window.solana connect→signMessage(bs58)→/auth flow; needsUsername→modal→/register; JWT+username in localStorage; persistent #wallet-bar (Connect / @user·Disconnect). getToken() is the Bearer source for net.ts. No wallet-adapter dep (vanilla TS).
    ...
    src/net.ts                apiFetch (auto-attaches Bearer JWT from auth.getToken) + typed fetchers + in-memory caches. Tier/TierTracks; MapEntry.tiers{...}. SubmitRunPayload (clientScore+finished) → SubmitRunResult{verifyStatus,serverScore,rankThisWindow?,rankAllTime?}. postNonce/postVerify/postRegister.
  ...
  api/                        Fastify on :8787 (ESM, tsx dev). Track pipeline + game endpoints live.
    ...
    src/auth.ts               Player wallet-auth primitives (P6): buildLoginMessage (exact signed string) / generateNonce / verifySignature (tweetnacl+bs58 ed25519) / signToken+verifyToken (JWT 7d {playerId,wallet} over JWT_SECRET) / requireAuth preHandler (Bearer→req.player, 401) / validateUsername (3-16 [a-z0-9_]). No private keys server-side. Separate from admin X-Admin-Key.
    ...
    src/routes/auth.ts        POST /nonce (upsert cr_auth_nonces, return message) / /verify (consumeNonce→ existing player JWT+last_login_at, or {needsUsername} for a new wallet — no row yet) / /register (fresh nonce + username → cr_players + JWT; 409 taken). consumeNonce = single-use + 5-min expiry.
    ...
    test/{trackgen,payouts,runVerify,auth}.test.ts  Vitest (52): trackgen goldens byte-identical + difficultyScore; payouts rankPool/computeWindowPayouts/closeWindow; runVerify anti-cheat (good log→verified; +5000 client→never verified; tampered log→server_score≠claim; DNF→no rank; 1255-pt replay <1s); auth message/username/sig-roundtrip.
