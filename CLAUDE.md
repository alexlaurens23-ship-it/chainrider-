...
apps/
  web/                        Vite + vanilla TS. Hash-routed SPA: #/ home, #/map/:slug/:period, #/ride/:trackId, #/playground.
    ...
    src/auth.ts               Account UI (P6-rev): persistent #wallet-bar (Log In / Sign Up — or @user·Log Out); LOGIN modal (username+4-digit PIN); 2-step SIGN-UP modal (username/PIN/wallet/confirm → review showing wallet BIG + clipboard-swap warning + required "I've checked" checkbox gating Create). bs58 client-side wallet pre-check. JWT+username in localStorage; getToken() is the Bearer source for net.ts. No Phantom/wallet-adapter (vanilla TS).
