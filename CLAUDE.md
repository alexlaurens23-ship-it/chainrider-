...
apps/
  ...
  api/                        Fastify on :8787 (ESM, tsx dev). Track pipeline + game endpoints live.
    ...
    src/auth.ts               JWT primitives (P6-rev): signToken+verifyToken (7d {playerId,username} over JWT_SECRET) / requireAuth preHandler (Bearer→req.player, 401) / validateUsername (3-16 [a-z0-9_]). Separate from admin X-Admin-Key. (P6 wallet-signature/nonce helpers removed; tweetnacl dropped.)
    src/accounts.ts           Account domain (P6-rev, pure + repo-injected like payouts.ts): validatePin (4 digits) / validateWalletAddress (bs58 → 32 bytes) / hashPin+verifyPin (bcryptjs cost 10 — PIN ONLY ever a hash) / evaluateLogin (pure lockout state machine: 5 wrong → lock LOCK_MS=15min; expired lock self-clears; active lock rejects even a correct PIN) / performSignup (validate+confirm-equality+uniqueness→createPlayer) / performLogin (generic 401 on unknown, 403 banned, 423 locked) / AccountRepo + createSupabaseAccountRepo. now injectable for tests.
    ...
    src/routes/runs.ts        POST /submit (requireAuth, P6): fast rejects (banned 403 / simVersion≠SIM_VERSION 400 / timeMs<min_run_time_ms 400 / inputLog>90000 400 / 1-per-10s/player 429) → getOrCreateOpenWindow (UTC-aligned 30-min bucket) → insert cr_runs pending → runSerial(verifyRun) on frozen points → store verify_status+server_score+finished → ranks (verified∧finished only) → {verifyStatus,serverScore,rankThisWindow?,rankAllTime?}. Logs verify durationMs (<1s).
    ...
    test/{trackgen,payouts,runVerify,accounts,auth}.test.ts  Vitest (67): trackgen goldens + difficultyScore; payouts; runVerify anti-cheat (good→verified; +5000 client→never verified; tampered log→server_score≠claim; DNF→no rank; 1255-pt replay <1s) UNCHANGED; accounts (signup rejects taken/reused-wallet/malformed/mismatch/short-PIN; PIN stored as bcrypt hash never plaintext; lockout after 5 + unlock after window via injected now); auth username + JWT roundtrip.
