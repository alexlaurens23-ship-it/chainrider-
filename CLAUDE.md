...
packages/
  physics/                    Deterministic sim + scoring. tsup dual ESM/CJS (dist/index.js + .cjs).
    ...
    src/sim.ts (control)      stepSim drive block: LOW-SPEED LAUNCH ASSIST (grounded+throttle+preSpeed<launchSpeedThreshold(3) → motorTorque ×launchBoost(1.8) scaling to ×1 above) + CONTEXT-AWARE REVERSE (S/down brakes when forwardVel>reverseEngageSpeed(1.5) or airborne — unchanged; else grounded crawl → rear motor reverses at reverseMotorTorque(30)/reverseMotorSpeed(12), capped at reverseMaxSpeed(4 m/s)). General handling otherwise locked.
    src/terrain.ts            Pure polyline geometry: y/slope at x, wrapAngle, swept head-circle death check.
    src/constants.ts          SIM_DT, gravity, iterations, SIM_VERSION=11 (bump on any physics/scoring change), lead-in/run-out/checkpoint/freeze constants.
    ...
    test/{grounded,scoring,respawn,launch,reverse}.test.ts  Vitest (13): incline climb + hard-landing + bit-identical replay; anti-exploit; respawn clearance; launch assist; context-aware reverse (forward brake decelerates > coast; standstill S backs up >3m, capped).
apps/
