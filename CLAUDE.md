...
apps/
  web/                        Vite + vanilla TS. Hash-routed SPA: #/ home, #/map/:slug/:period, #/ride/:trackId, #/playground.
    ...
    src/net.ts                apiFetch (auto-attaches Bearer JWT from auth.getToken; surfaces server {error} as ApiError.message for lockout text) + typed fetchers + in-memory caches. Tier/TierTracks; MapEntry.tiers{...}. SubmitRunPayload (clientScore+finished) → SubmitRunResult{verifyStatus,serverScore,rankThisWindow?,rankAllTime?}. postSignup/postLogin.
  ...
  api/                        Fastify on :8787 (ESM, tsx dev). Track pipeline + game endpoints live.
    ...
    src/runVerify.ts          PURE server-authoritative verify (P6): verifyRun({points,parTimeMs,inputLog,client[,maxTicks]}) re-runs simulateReplay(DEFAULT_TUNE) → compares: exact (score ±1% ∧ flips/crashes/finished == ∧ time ±250ms)→'verified'; ≤±5%→'flagged'; worse/throw→'failed'. eligibleForRank=verified∧finished. No DB (route wraps it). server_score is the only ranking number.
    ...
    src/routes/runs.ts        POST /submit (requireAuth, P6/P6b): fast rejects (banned 403 / simVersion 400 / min_run_time_ms 400 / inputLog>90000 400 / 1-per-10s 429) → **queue-cap 503 (MAX_VERIFY_INFLIGHT 25, M4)** → **load track + reject non-active 409 (M7)** → reserve slot (try/finally) → getOrCreateOpenWindow (**upsert onConflict starts_at + status='open' guard; closed→null window, M3**) → insert pending → runSerial(verifyRun) → **per-run budget: durationMs>2s ⇒ failed (M4)** → store verify_status+server_score+finished → ranks (verified∧finished only). server_score is the only ranking number.
    src/routes/leaderboards.ts GET /:trackId → [] until P7.
    ...
    test/{trackgen,payouts,runVerify,accounts,auth}.test.ts  Vitest (69): trackgen goldens; payouts; runVerify anti-cheat (good→verified; +5000→never verified; tampered log→score≠claim; DNF→no rank; <1s) UNCHANGED; accounts (signup rejections; PIN=bcrypt-hash-never-plaintext; lockout+unlock; **H1 atomic lockout holds under 30 concurrent wrong-PINs**; **M6 escalation 15→30→60min**); auth username + HS256 JWT roundtrip.
...
- **P7**: real per-tier leaderboards (`/api/leaderboards/:trackId`, currently `[]`) reading verified `cr_runs` + the payout-window cron calling `closeWindow` (P4.6 `payouts.ts`). Run submission + server re-sim verification is DONE (P6).
